---
slug: distributed-transaction-notes
title: 分布式事务
authors:
  name: zhoujun134
...
事务的执行必须保证系统的一致性，在事务开始之前和事务结束以后，数据库的完整性没有被破坏，就拿转账为例，A有500元，B有500元，如果在一个事务里A成功转给B 50元，那么不管发生什么，那么最后A账户和B账户的数据之和必须是1000元。
...
在数据库系统中，既有存放数据的文件，也有存放日志的文件。日志在内存中也是有缓存`Log buffer`，也有磁盘文件`log file`。

`MySQL`中的日志文件，有这么两种与事务有关：undo日志与redo日志。
...
`Undo Log`的原理很简单，为了满足事务的原子性，在操作任何数据之前，首先将数据备份到Undo Log。然后进行数据的修改。如果出现了错误或者用户执行了ROLLBACK语句，系统可以利用Undo Log中的备份将数据恢复到事务开始之前的状态。
...
和`Undo Log`相反，`Redo Log`记录的是**新数据**的备份。在事务提交前，只要将`Redo Log`持久化即可，不需要将数据持久化，减少了`IO`的次数。
...
![image-20200304201018438](https://img.zbus.top//zbus/blog/202312050821799.webp)
...
![image-20200304202639509](https://img.zbus.top//zbus/blog/202312050822518.webp)
...
![image-20200304204442839](https://img.zbus.top//zbus/blog/202312050822971.webp)
...
![image-20200304205842784](https://img.zbus.top//zbus/blog/202312050822921.webp)
...
![image-20200304210120471](https://img.zbus.top//zbus/blog/202312050822628.webp)
...
![image-20200304210414309](https://img.zbus.top//zbus/blog/202312050822222.webp)

接下来，用户的读操作就会得到 v1。这就叫一致性。

![image-20200304210506575](https://img.zbus.top//zbus/blog/202312050822606.webp)

问题是，用户有可能向 G2 发起读操作，由于 G2 的值没有发生变化，因此返回的是 v0。G1 和 G2 读操作的结果不一致，这就不满足一致性了。

![image-20200304210521364](https://img.zbus.top//zbus/blog/202312050822571.webp)

为了让 G2 也能变为 v1，就要在 G1 写操作的时候，让 G1 向 G2 发送一条消息，要求 G2 也改成 v1。

![image-20200304210540168](https://img.zbus.top//zbus/blog/202312050822704.webp)

这样的话，用户向 G2 发起读操作，也能得到 v1。

![image-20200304210557117](https://img.zbus.top//zbus/blog/202312050822846.webp)
...
![image-20200305141029973](https://img.zbus.top//zbus/blog/202312050823421.webp)
...
![image-20200305141318326](https://img.zbus.top//zbus/blog/202312050823892.webp)
...
- 单点故障问题
  ...
  ![image-20200305142812815](https://img.zbus.top//zbus/blog/202312050823439.webp)

  假设`coordinator`和`voter3`都在`Commit`这个阶段`crash`了, 而`voter1`和`voter2`没有收到commit消息. 这时候voter1和voter2就陷入了一个困境. 因为他们并不能判断现在是两个场景中的哪一种:
...
![image-20200305155521612](https://img.zbus.top//zbus/blog/202312050823998.webp)
...
![image-20200305155830732](https://img.zbus.top//zbus/blog/202312050823580.webp)
...
![image-20200305181454125](https://img.zbus.top//zbus/blog/202312050823924.webp)
...
![image-20200305183431211](https://img.zbus.top//zbus/blog/202312050823643.webp)
...
![image-20200305200131083](https://img.zbus.top//zbus/blog/202312050823337.webp)



一次消息发送的时序图：

![image-20200305205430863](https://img.zbus.top//zbus/blog/202312050823285.webp)
...
![image-20200305212340203](https://img.zbus.top//zbus/blog/202312050823120.webp)
...
![image-20200305213652558](https://img.zbus.top//zbus/blog/202312050824899.webp)
...
![image-20200305214649845](https://img.zbus.top//zbus/blog/202312050824094.webp)
...
![image-20200305225811888](https://img.zbus.top//zbus/blog/202312050824787.webp)
...
![Saga 模式](https://img.zbus.top//zbus/blog/202312050824372.webp)
...
![Seata](https://img.zbus.top//zbus/blog/202312050824420.webp)

### Seata 产品模块

如下图所示，Seata 中有三大模块，分别是 TM、RM 和 TC。 其中 TM 和 RM 是作为 Seata 的客户端与业务系统集成在一起，TC 作为 Seata 的服务端独立部署。

![image-20200305225811888](https://img.zbus.top//zbus/blog/202312050824468.webp)



### Seata支持的事务模型

Seata 会有 4 种分布式事务解决方案，分别是 AT 模式、TCC 模式、Saga 模式和 XA 模式。![image-20200305230513415](https://img.zbus.top//zbus/blog/202312050824333.webp)
...
![image-20200306164728739](https://img.zbus.top//zbus/blog/202312050824457.webp)
...
![image-20200306170419354](https://img.zbus.top//zbus/blog/202312050824361.webp)

找到项目所在目录，选中并打开：

![image-20200306170520985](https://img.zbus.top//zbus/blog/202312050824157.webp)

项目结构如下：

![image-20200306171827203](https://img.zbus.top//zbus/blog/202312050825892.webp)
...
![image-20200306173439268](https://img.zbus.top//zbus/blog/202312050825822.webp)

库存：

![image-20200306173511332](https://img.zbus.top//zbus/blog/202312050825332.webp)
...
![image-20200306173343839](https://img.zbus.top//zbus/blog/202312050825735.webp)

查看数据库数据：

余额：

![image-20200306173602942](https://img.zbus.top//zbus/blog/202312050825147.webp)

库存：

![image-20200306173629491](https://img.zbus.top//zbus/blog/202312050825125.webp)

订单：

![image-20200306173700813](https://img.zbus.top//zbus/blog/202312050825221.webp)
...
![image-20200306173916953](https://img.zbus.top//zbus/blog/202312050825826.webp)

看下用户余额：

![image-20200306224048175](https://img.zbus.top//zbus/blog/202312050825266.webp)

因为扣款失败，因此这里没有扣减

来看下库存数据：

![image-20200306174001901](https://img.zbus.top//zbus/blog/202312050825241.webp)
...
![image-20200306174740064](https://img.zbus.top//zbus/blog/202312050825501.webp)



然后解压即可，其目录结构如下：

![image-20200306174818712](https://img.zbus.top//zbus/blog/202312050826079.webp)
...
![image-20200306201749660](https://img.zbus.top//zbus/blog/202312050826248.webp)
...
![image-20200306223043452](https://img.zbus.top//zbus/blog/202312050826193.webp)
...
![image-20200306173916953](https://img.zbus.top//zbus/blog/202312050826737.webp)

看下用户余额：

![image-20200306224048175](https://img.zbus.top//zbus/blog/202312050826190.webp)

因为扣款失败，因此这里没有扣减

来看下库存数据：

![image-20200306174001901](https://img.zbus.top//zbus/blog/202312050826274.webp)
